...
def load_hitter_standard_period(con, season, period_num):
    """Load one period of hitter standard stats into hitter_period_stats_standard."""
    
    file_path = RAW_DIR / str(season) / f"p{period_num}_batter_standard.csv"
    ...
    # Convert numeric columns
    num_cols = ['AB', 'R', 'H', 'singles', 'doubles', 'triples',
                'HR', 'RBI', 'BB', 'KO', 'SB', 'CS', 'AVG', 'OBP', 'SLG']
    for col in num_cols:
        df[col] = pd.to_numeric(df[col], errors='coerce')
...
def load_pitcher_standard_period(con, season, period_num):
    """Load one period of pitcher standard stats into pitcher_period_stats_standard."""
    
    file_path = RAW_DIR / str(season) / f"p{period_num}_pitcher_standard.csv"
    ...
    df = df[df['avail'].notna()].copy()
    df = df[df['player'].notna()].copy()
    df = df[df['INNs'] != 'INNs'].copy()
    
    # Convert numeric columns
    num_cols = ['INNs', 'APP', 'GS', 'QS', 'CG', 'W', 'L',
                'SV', 'BS', 'HD', 'K', 'BB', 'H', 'ERA', 'WHIP']
    for col in num_cols:
        df[col] = pd.to_numeric(df[col], errors='coerce')
